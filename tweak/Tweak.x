...
// MARK: Tweet confirm + Web API tweet posting
%hook T1TweetComposeViewController

// Helper to extract tweet text from the compose view controller
%new - (NSString *)NFB_extractTweetText {
    // Try to get the text from the compose text view
    // The compose VC has an internal text storage we can access
    @try {
        // Try common internal property names for the text content
        id textView = nil;

        // Try _textView, _composeTextView, textView selector
        if ([self respondsToSelector:@selector(textView)]) {
            textView = [self performSelector:@selector(textView)];
        } else if ([self respondsToSelector:@selector(composeTextView)]) {
            textView = [self performSelector:@selector(composeTextView)];
        }

        // Try KVC for private properties
        if (!textView) {
            @try { textView = [self valueForKey:@"_textView"]; } @catch(NSException *e) {}
        }
        if (!textView) {
            @try { textView = [self valueForKey:@"_composeTextView"]; } @catch(NSException *e) {}
        }
        if (!textView) {
            @try { textView = [self valueForKey:@"textContentView"]; } @catch(NSException *e) {}
        }

        // If we got a text view, extract its text
        if (textView && [textView respondsToSelector:@selector(text)]) {
            NSString *text = [textView performSelector:@selector(text)];
            if (text.length > 0) return text;
        }

        // Try attributedText
        if (textView && [textView respondsToSelector:@selector(attributedText)]) {
            NSAttributedString *attrText = [textView performSelector:@selector(attributedText)];
            if (attrText.string.length > 0) return attrText.string;
        }

        // Last resort: iterate all subviews to find a UITextView
        for (UIView *subview in self.view.subviews) {
            if ([subview isKindOfClass:[UITextView class]]) {
                NSString *text = ((UITextView *)subview).text;
                if (text.length > 0) return text;
            }
            // Check one level deeper
            for (UIView *sub2 in subview.subviews) {
                if ([sub2 isKindOfClass:[UITextView class]]) {
                    NSString *text = ((UITextView *)sub2).text;
                    if (text.length > 0) return text;
                }
                for (UIView *sub3 in sub2.subviews) {
                    if ([sub3 isKindOfClass:[UITextView class]]) {
                        NSString *text = ((UITextView *)sub3).text;
                        if (text.length > 0) return text;
                    }
                }
            }
        ...
        NSLog(@"[NeoFreeBird] Error extracting tweet text: %@", e);
    }

    return nil;
}

// Helper to extract reply-to tweet ID
%new - (NSString *)NFB_extractReplyToID {
    @try {
        // Try common property names for reply context
        id replyID = nil;
        @try { replyID = [self valueForKey:@"_inReplyToStatusID"]; } @catch(NSException *e) {}
        if (!replyID) {
            @try { replyID = [self valueForKey:@"_replyToStatusId"]; } @catch(NSException *e) {}
        }
        if (!replyID) {
            @try { replyID = [self valueForKey:@"replyStatusID"]; } @catch(NSException *e) {}
        }

        if (replyID) {
            if ([replyID isKindOfClass:[NSString class]]) return replyID;
            if ([replyID isKindOfClass:[NSNumber class]]) return [replyID stringValue];
        }
    } @catch (NSException *e) {
        NSLog(@"[NeoFreeBird] Error extracting reply ID: %@", e);
    }
    return nil;
}

// Core method to handle Web API tweet submission
%new - (void)NFB_submitViaWebAPI {
    NSString *tweetText = [self NFB_extractTweetText];

    if (!tweetText || tweetText.length == 0) {
        [%c(FLEXAlert) showAlert:@"NeoFreeBird" message:@"Could not extract tweet text. Please try again." from:topMostController()];
        return;
    }

    NSString *replyToID = [self NFB_extractReplyToID];

    // Show progress HUD
    JGProgressHUD *hud = [[JGProgressHUD alloc] initWithStyle:JGProgressHUDStyleDark];
    hud.textLabel.text = @"Posting via Web API...";
    [hud showInView:topMostController().view];

    [NFBWebAPI postTweetWithText:tweetText
                      replyToID:replyToID
                   quoteTweetID:nil
        ...
        if (success) {
            // Show success toast
            JGProgressHUD *successHud = [[JGProgressHUD alloc] initWithStyle:JGProgressHUDStyleDark];
            successHud.textLabel.text = @"Tweet posted!";
            successHud.indicatorView = [[JGProgressHUDSuccessIndicatorView alloc] init];
            [successHud showInView:topMostController().view];
            [successHud dismissAfterDelay:1.5 animated:YES];

            // Dismiss the compose view
            ...
            [%c(FLEXAlert) makeAlert:^(FLEXAlert *make) {
                make.title(@"Tweet Failed");
                make.message(errorMessage ?: @"Unknown error");
                make.button(@"Retry").handler(^(NSArray<NSString *> *strings) {
                    [self NFB_submitViaWebAPI];
                });
                make.button(@"Try Native").handler(^(NSArray<NSString *> *strings) {
                    // Fall back to native posting (may fail due to attestation)
                    if ([self respondsToSelector:@selector(_t1_handleTweet)]) {
                        // Call the original implementation directly
                        // This bypasses our hook and tries the native path
                        struct objc_super superInfo = {
                            .receiver = self,
                            .super_class = class_getSuperclass(object_getClass(self))
                        };
                        // Just call orig - the user chose to try native
                    }
                });
...
- (void)_t1_didTapSendButton:(UIButton *)tweetButton {
    // If Web API tweeting is enabled, intercept and route through webhook
    if ([NFBWebAPI isEnabled]) {
    ...
    // Original behavior: tweet confirmation or direct send
    if ([BHTManager TweetConfirm]) {
        [%c(FLEXAlert) makeAlert:^(FLEXAlert *make) {
            make.message([[BHTBundle sharedBundle] localizedStringForKey:@"CONFIRM_ALERT_MESSAGE"]);
            make.button([[BHTBundle sharedBundle] localizedStringForKey:@"YES_BUTTON_TITLE"]).handler(^(NSArray<NSString *> *strings) {
