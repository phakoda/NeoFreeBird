...
@interface WebAPISettingsViewController : UIViewController
- (instancetype)initWithAccount:(TFNTwitterAccount *)account;
@end
...
- (void)showWebAPISettings {
    WebAPISettingsViewController *vc = [[WebAPISettingsViewController alloc] initWithAccount:self.account];
    [self.navigationController pushViewController:vc animated:YES];
}
...
// ==============================
// WebAPISettingsViewController
// ==============================
@interface WebAPISettingsViewController () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate>
@property (nonatomic, strong) TFNTwitterAccount *account;
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) UISwitch *enableSwitch;
@property (nonatomic, strong) UITextField *consumerKeyField;
@property (nonatomic, strong) UITextField *consumerSecretField;
@property (nonatomic, strong) UITextField *accessTokenField;
@property (nonatomic, strong) UITextField *accessSecretField;
@end

@implementation WebAPISettingsViewController
...
- (void)setupNav {
    NSString *title = @"Web API Tweeting";
    if (self.account) {
        self.navigationItem.titleView = [objc_getClass("TFNTitleView") titleViewWithTitle:title subtitle:self.account.displayUsername];
    } else {
        self.title = title;
...
#pragma mark - Credential Field Builder
...
- (UITableViewCell *)credentialCellWithLabel:(NSString *)labelText
    ...
    field.font = [fontGroup performSelector:@selector(subtext2Font)];
    ...
    [NSLayoutConstraint activateConstraints:@[
        [label.leadingAnchor constraintEqualToAnchor:cell.contentView.leadingAnchor constant:20],
        ...
        [field.leadingAnchor constraintEqualToAnchor:cell.contentView.leadingAnchor constant:20],
        [field.trailingAnchor constraintEqualToAnchor:cell.contentView.trailingAnchor constant:-20],
        ...
        [field.heightAnchor constraintGreaterThanOrEqualToConstant:30]
    ]];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    return 3; // Section 0: Enable toggle, Section 1: Credentials, Section 2: Test button
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (section == 0) return 1; // Enable toggle
    if (section == 1) return 4; // 4 credential fields
    return 1; // Test Credentials button
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        cell.textLabel.text = @"Enable Direct API Tweeting";
        cell.textLabel.font = [fontGroup performSelector:@selector(bodyBoldFont)];
        cell.textLabel.textColor = textColor;
        cell.detailTextLabel.text = @"Post tweets via Twitter API v2 directly";
        ...
        self.enableSwitch.on = [[NSUserDefaults standardUserDefaults] boolForKey:@"webapi_tweet_enabled"];
        ...
        switch (indexPath.row) {
            case 0:
                return [self credentialCellWithLabel:@"API Key (Consumer Key)"
                                        placeholder:@"Enter your API Key"
                                          defaultsKey:@"webapi_consumer_key"
                                           textField:&_consumerKeyField
                                               tag:100
                                            secure:NO];
            case 1:
                return [self credentialCellWithLabel:@"API Key Secret"
                                        placeholder:@"Enter your API Secret"
                                          defaultsKey:@"webapi_consumer_secret"
                                           textField:&_consumerSecretField
                                               tag:101
                                            secure:YES];
            case 2:
                return [self credentialCellWithLabel:@"Access Token"
                                        placeholder:@"Enter your Access Token"
                                          defaultsKey:@"webapi_access_token"
                                           textField:&_accessTokenField
                                               tag:102
                                            secure:NO];
            case 3:
                return [self credentialCellWithLabel:@"Access Token Secret"
                                        placeholder:@"Enter your Access Token Secret"
                                          defaultsKey:@"webapi_access_secret"
                                           textField:&_accessSecretField
                                               tag:103
                                            secure:YES];
        }
    }
        ...
        UITableViewCell *cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
        cell.backgroundColor = [BHDimPalette currentBackgroundColor];
        cell.textLabel.text = @"Test Credentials";
        cell.textLabel.textColor = BHTCurrentAccentColor();
        id fontGroup = [objc_getClass("TAEStandardFontGroup") sharedFontGroup];
        cell.textLabel.font = [fontGroup performSelector:@selector(bodyBoldFont)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2 && indexPath.row == 0) {
        ...
        [NFBWebAPI testCredentials:^(BOOL success, NSString *message) {
            [hud dismiss];

            JGProgressHUD *resultHud = [[JGProgressHUD alloc] initWithStyle:JGProgressHUDStyleDark];
            if (success) {
                resultHud.textLabel.text = @"Success!";
                resultHud.detailTextLabel.text = message;
                resultHud.indicatorView = [[JGProgressHUDSuccessIndicatorView alloc] init];
            } else {
                resultHud.textLabel.text = @"Failed";
                resultHud.detailTextLabel.text = message;
                resultHud.indicatorView = [[JGProgressHUDErrorIndicatorView alloc] init];
            }
            [resultHud showInView:self.view];
            [resultHud dismissAfterDelay:2.5 animated:YES];
        }];
    }
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 0) {
        ...
        UILabel *label = [[UILabel alloc] init];
        label.translatesAutoresizingMaskIntoConstraints = NO;
        label.text = @"Post tweets directly via the Twitter API v2 to bypass attestation checks on modified clients. No external server needed.";
        label.numberOfLines = 0;
        id fontGroup = [objc_getClass("TAEStandardFontGroup") sharedFontGroup];
        label.font = [fontGroup performSelector:@selector(subtext2Font)];
        Class TAEColorSettingsCls = objc_getClass("TAEColorSettings");
    ...
    if (section == 1) {
        ...
        UILabel *label = [[UILabel alloc] init];
        label.translatesAutoresizingMaskIntoConstraints = NO;
        label.text = @"Enter your Twitter API v2 credentials from developer.x.com. Your app must have Read+Write permissions.";
        label.numberOfLines = 0;
        id fontGroup = [objc_getClass("TAEStandardFontGroup") sharedFontGroup];
        label.font = [fontGroup performSelector:@selector(subtext2Font)];
        Class TAEColorSettingsCls = objc_getClass("TAEColorSettings");
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if (section == 0 || section == 1) return UITableViewAutomaticDimension;
    return CGFLOAT_MIN;
}

- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    if (section == 2) {
        return @"Credentials are stored locally on your device and used only to sign API requests.";
    }
    return nil;
}
...
- (void)enableSwitchChanged:(UISwitch *)sender {
    [[NSUserDefaults standardUserDefaults] setBool:sender.isOn forKey:@"webapi_tweet_enabled"];
}

- (void)saveFields {
    if (self.consumerKeyField.text) {
        [[NSUserDefaults standardUserDefaults] setObject:self.consumerKeyField.text forKey:@"webapi_consumer_key"];
    }
    if (self.consumerSecretField.text) {
        [[NSUserDefaults standardUserDefaults] setObject:self.consumerSecretField.text forKey:@"webapi_consumer_secret"];
    }
    if (self.accessTokenField.text) {
        [[NSUserDefaults standardUserDefaults] setObject:self.accessTokenField.text forKey:@"webapi_access_token"];
    }
    if (self.accessSecretField.text) {
        [[NSUserDefaults standardUserDefaults] setObject:self.accessSecretField.text forKey:@"webapi_access_secret"];
    }
}
